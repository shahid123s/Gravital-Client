    ...
    <tbody>
      {dataCollection.length > 0 ? dataCollection.map((data, index) => (
        <tr key={(data._id)} className="border-b border-gray-700 ">
          ...
          <td className="px-4 py-2" onClick={() => hanldeClick(data._id, data.username ? 'user' : data.type ? false: 'post')}>{data.fullName || data.userID?.username || data.type === 'user' ? <p>user <span className= 'text-gray-400'>{`( ${data.details.username} ) `}</span></p>: 'post' } </td>
